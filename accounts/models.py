...
# Signals to automatically create and update the Profile when a User is created/updated
...
class ReminderPreference(models.Model):
    """User preferences for appointment reminders"""
...
class InsuranceProvider(models.Model):
    """Insurance providers available in the system"""
...
class InsurancePolicy(models.Model):
    ...
    user = models.OneToOneField(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name='insurance_policy')
    ...
    primary_holder_relationship = models.CharField(
        max_length=20,
        choices=[
            ('self', 'Self'),
            ('spouse', 'Spouse'),
...
class ServiceType(models.Model):
    """Types of medical services with their codes and costs"""
    name = models.CharField(max_length=100)
...
class BillingRecord(models.Model):
    """Record of billable services for a patient"""
...
class BillingService(models.Model):
    """Many-to-many relationship between BillingRecord and ServiceType with quantity and price"""
...
class Payment(models.Model):
    """Records individual payments made against billing records"""
